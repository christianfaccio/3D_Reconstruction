...
To conclude this section, we will introduce a collection of recent datasets for the purpose of training and validating 3D reconstruction models. The idea is to provide the reader with context on what to find in a dataset, as well as introduce potential candidates for Section 5.
...
\textbf{TODO}: Explain a bit more how and where each framework integrates gaussian splatting.For now, just described and summarized some of the recent work.
...
\begin{itemize}
	\item \textit{RGB-IR fusion}. The BiNAR framework allows the joint scene reconstruction of non aligned RGB and IR data. This can be applied to unmanned aerial night-time inspection and post-disaster thermal mapping, where thermal information is crucial, but lacks the resolution and texture of RGB images.
	\item \textit{Temperature modeling}. The authors of the framework created and released a new dataset that supports the recovery of real world temperature values at pixel level, aligned with the high resolution 3D geometry.
\end{itemize}

\textbf{TODO}: Explain where and how Gaussian Splatting is applied in BiNAR
...
\textbf{TODO}: Explain where and how Gaussian Splatting is applied in DroneSplat
...
\textbf{TODO}: Explain where and how Gaussian Splatting is applied in SR3S and (optionally) MeshSplat
...
\end{itemize}
\textbf{TODO}: Explain where and how Gaussian Splatting is applied in \cite{powergrid_3dgs}
...
\end{itemize}
\textbf{TODO}: Explain where and how Gaussian Splatting is applied in \cite{digitaltwin_damage}
